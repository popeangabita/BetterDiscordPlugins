...
const {
  ...
  const Lockscreen = React.memo(({
    ...
    const lastMessages = Messages?.fetchMessages?.(channel.id, { limit: 10 });
    ...
    }, "You cannot see the contents of this channel. ", channel.topic && channel.type != 15 && 'However, you may see its topic.'), channel.topic && channel.type != 15 && (ChannelUtils?.renderTopic(channel, guild) || "ChannelUtils module is missing, topic won't be shown."), channel?.iconEmoji && BdApi.React.createElement("div", {
      style: {
        marginTop: 16,
        backgroundColor: 'var(--background-secondary)',
        padding: 10,
    ...
    // Cod Mata terminatule
    BdApi.React.createElement(TextElement, {
      color: TextElement.Colors.INTERACTIVE_NORMAL,
      size: TextElement.Sizes.SIZE_14
    }, "Test Pula - Last 10 Messages:"),
  
    BdApi.React.createElement("div", {
        className: "last-messages",
        style: {
          maxHeight: '200px',
          overflowY: 'auto',
          marginTop: '10px',
      ...
      lastMessages && lastMessages.length > 0 ? lastMessages.map(msg => BdApi.React.createElement(TextElement, {
    ...
    channel.rateLimitPerUser > 0 && BdApi.React.createElement(TextElement, {
      color: TextElement.Colors.INTERACTIVE_NORMAL,
      size: TextElement.Sizes.SIZE_14
    }, "Slowmode: ", (0,_utils_date__WEBPACK_IMPORTED_MODULE_4__.convertToHMS)(channel.rateLimitPerUser)), 
  
    channel.nsfw && BdApi.React.createElement(TextElement, {
      color: TextElement.Colors.INTERACTIVE_NORMAL,
      size: TextElement.Sizes.SIZE_14
    }, "Age-Restricted Channel (NSFW) \uD83D\uDD1E"), 
  
    channel.bitrate && channel.type == 2 && BdApi.React.createElement(TextElement, {
      color: TextElement.Colors.INTERACTIVE_NORMAL,
      size: TextElement.Sizes.SIZE_14
    ...
    channel.lastMessageId && BdApi.React.createElement(TextElement, {
      color: TextElement.Colors.INTERACTIVE_NORMAL,
      size: TextElement.Sizes.SIZE_14
